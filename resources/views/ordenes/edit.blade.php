...
<div class="container">
    ...
    <form action="{{ route('ordenes.update', $orden->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <textarea name="observaciones" class="form-control">{{ old('observaciones', $orden->observaciones) }}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            @foreach($tecnicos as $tecnico)
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="tecnicos[]" value="{{ $tecnico->id }}"
                        {{ in_array($tecnico->id, old('tecnicos', $orden->tecnicos->pluck('id')->toArray())) ? 'checked' : '' }}>
        ...
        <div class="mb-3">
            ...
            @php
                // Pre-cargar las cantidades existentes en un array
                $cantidades_actuales = $orden->materiales->pluck('pivot.cantidad', 'id')->toArray();
            @endphp
            
            @foreach($materiales as $material)
                @php
                    // Obtener la cantidad guardada o 0 por defecto
